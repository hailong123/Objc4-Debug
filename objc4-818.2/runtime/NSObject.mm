...
struct SideTable {
    spinlock_t slock;//自旋锁 用于 上锁/解锁 SideTable
    RefcountMap refcnts; //用来存储 OC 对象 的引用计数的 hash表(仅在未开启isa优化或在isa优化情况下isa_t的引用计数溢出时才会用到)
    weak_table_t weak_table;//存储对象弱引用指针的hash表, 是OC中 weak 功能实现的核心数据结构
...
/*
 storeWeak方法实际上是接收了5个参数，
 分别是haveOld、haveNew和crashIfDeallocating ，
 这三个参数都是以模板的方式传入的，是三个bool类型的参数。
 分别表示weak指针之前是否指向了一个弱引用，weak指针是否需要指向一个新的引用，若果被弱引用的对象正在析构，此时再弱引用该对象是否应该crash。
...
storeWeak(id *location, objc_object *newObj)
{
 ...
 retry:
    if (haveOld) { //如果 weak ptr 之间弱引用过一个 obj, 则将这obj所对应的 SideTable 取出, 赋值给 oldTable
        oldObj = *location;
        oldTable = &SideTables()[oldObj];
    } else {
        oldTable = nil; //如果 weak ptr 之前没有弱引用过一个 obj 则 oldTable = nil
    }
    if (haveNew) { //如果 weak ptr 要 weak 引用一个新的obj 则将该 obj 对应的 SideTable 取出, 赋值给newTable
        newTable = &SideTables()[newObj];
    } else {
        newTable = nil;//如果weak ptr 不需要引用一个新 obj 则newTable = nil
    }

    //加锁 防止 竞争冲突
    SideTable::lockTwo<haveOld, haveNew>(oldTable, newTable);

    //location 应该与 oldObj 保持一致 如果不同 说明当前的 location 已经处理过 oldObj 可能又被其他线程所修改
    ...
    if (haveNew  &&  newObj) {
        ...
        if (cls != previouslyInitializedClass  &&  
            !((objc_class *)cls)->isInitialized()) //如果 cls 还没有初始化 先初始化 再尝试设置 weak
        {
            SideTable::unlockTwo<haveOld, haveNew>(oldTable, newTable);
            ...
            previouslyInitializedClass = cls; //这里记录一下 previouslyInitializedClass 防止 改if 分支再次进入

            goto retry; //重新获取一遍 newObj 这时的newObj应该已经初始化过了
        }
    }

    // Clean up old value, if any.
    if (haveOld) {
        weak_unregister_no_lock(&oldTable->weak_table, oldObj, location);//如果 weak_ptr之前弱引用过别的对象 oldObj, 则调用weak_unregister_no_lock 在oldObj的weak_entry_t中移除该weak_ptr地址
    }

    // Assign new value, if any.
    if (haveNew) {//如果weak_ptr需要弱引用新的对象 newObj
        //(1)调用weak_register_no_lock方法, 将weak ptr 的地址记录到newObj对应的weak_entry_t中
        newObj = (objc_object *)
        ...
        //(2)更新newObj的isa的weakly_referenced bit 标志位
        if (!newObj->isTaggedPointerOrNil()) {
            newObj->setWeaklyReferenced_nolock();
        }

        // Do not set *location anywhere else. That would introduce a race.
        // (3) *location 赋值 也就是将weak ptr直接指向了 newObj 可以看到 这里并没有将 newObj的引用计数+1
        *location = (id)newObj; //将weak ptr 指向 object
    }
    else {
    ...
    return (id)newObj; //返回newObj 此时的newObj与刚传入时相比, weakly-refreenced bit 位置置为 1
}
...
objc_object::clearDeallocating_slow()
{
    ...
    SideTable& table = SideTables()[this];//在全局的SideTables 中 ,以this指针为key, 找到对应的SideTable
    table.lock();
    if (isa.weakly_referenced) {//如果obj被弱引用
        weak_clear_no_lock(&table.weak_table, (id)this);//在SideTable的weak_table中对this进行清理操作
    }
